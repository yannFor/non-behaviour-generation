...
def plotconsistanceLossEpoch(num_epoch,loss,t_loss=None):
    ...
    ax1.set_ylabel("Loss")
    ax1.legend()
    plt.savefig(constants.saved_path + f'lossConsistance_epoch_{num_epoch}.png')
    plt.close()
...
def plotHistAllRLossEpoch(num_epoch, loss_eye, loss_pose_r, loss_au):
    fig = plt.figure(dpi=100)
    ax1 = fig.add_subplot(1, 1, 1)
    ...
    ax1.set_ylabel("Loss")
    ax1.legend()
    plt.savefig(constants.saved_path + f'all_Rloss_epoch_{num_epoch}.png')
    plt.close()
