...
def create_set(dataset_name, output_path, visual_path, audio_path, collection, type_of_set, timestep):
    ...
    for key in collection:
        ...
        if(os.path.isfile(path_df_audio) and os.path.isfile(path_df_visual)):
            ...
            #index_not_move = [i for i in index_not_move if i <= len(df_video)-1]
    ...
    with open(output_path + "X_"+type_of_set+"_"+dataset_name+"_speakerOnly.p", 'wb') as f:
        pickle.dump(X, f)
    with open(output_path + "y_"+type_of_set+"_"+dataset_name+"_speakerOnly.p", 'wb') as f:
        pickle.dump(Y, f)
    with open(output_path + "intervals_"+type_of_set+"_"+dataset_name+"_speakerOnly.p", 'wb') as f:
...
def create_final_y_test(test_key, processed_video_path, output_path, dataset_name):
        ...
        output_path = output_path + "y_test_final_"+dataset_name+"_speakerOnly.p"
        all_df = []
...
def main():
    ...
    for set_name in sets:
        ...
        create_set(dataset_name, output_path, visual_path, audio_path, keys, set_name, timestep)
        print("*"*10, set_name, " set created", "*"*10)

        if(set_name == "test"):
            create_final_y_test(keys, visual_path, output_path, dataset_name)
