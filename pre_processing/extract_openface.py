...
#calculate transition betwwen two given timestep where confidence to low
def calculateTransition(df, low_index, high_index):
    new_df = df.copy()
    number_of_value = high_index - low_index
    step = (new_df.iloc[high_index] - new_df.iloc[low_index]) / number_of_value
    old = new_df.iloc[low_index]
    for idx in range(low_index+1, high_index):
        new_line = old + step
        new_df.iloc[idx] = new_line
        old = new_line
    return new_df
    ...
    for csv_file in listdir(out):
        if(".csv" in csv_file):
            ...
            for index in index_to_modify:
                df_video = calculateMissingTransition(df_video, index, timestep)

            # if confidence openFace < min_confidence 
            # --> we look for the closest confident values and we calculate the transition between these two confident values
            max_index = len(df_video) - 1
            index = 1
            while(index < max_index):
                if df_video.at[index, "confidence"]  < min_confidence:
                    good_confidence_index_smaller = index-1
                    good_confidence_index_higer = index+1
                    while(df_video.at[good_confidence_index_smaller, "confidence"] < min_confidence and good_confidence_index_smaller != 0):
                        good_confidence_index_smaller = good_confidence_index_smaller - 1
                    while(df_video.at[good_confidence_index_higer, "confidence"] < min_confidence and good_confidence_index_higer != max_index):
                        good_confidence_index_higer = good_confidence_index_higer + 1
                    index = good_confidence_index_higer
                    df_video = calculateTransition(df_video, good_confidence_index_smaller, good_confidence_index_higer)
                else:
                    index = index + 1
