...
def addSpeakOrNotFeatures(out, path, anno_dir, timestep, cut_dictionnary):
...
if __name__ == "__main__":
    ...
    without_extract = sys.argv[2] ## if true, reuse the existing csv file for removeOverlapAndAlignStep and addDerivative
    ...
    timestep = 0.04
    path, processed_dir, audio_dir, anno_dir, set = getPath(dataset_name)
        ...
        addDerivative(out, audio_features[1:])
         ...
         for set_name in set:
            ...
            if(without_extract == "True"):
                createCsvFile(dir, out + "origin/")
            removeOverlapAndAlignStep(out, timestep)
            addSpeakOrNotFeatures(out, path, anno_dir, timestep, cut_dictionnary)
            addDerivative(out, audio_features[1:])